...
void GCS_MAVLINK_Plane::send_attitude() const
{
    ...
    // Gerçek değerleri al
    float true_roll = ahrs.get_roll();
    float true_pitch = ahrs.get_pitch();
    float y = ahrs.get_yaw();

    // Trim, gerçek pitch'e uygulanmalı
    if (!(plane.flight_option_enabled(FlightOptions::GCS_REMOVE_TRIM_PITCH))) {
        true_pitch -= radians(plane.g.pitch_trim);
    }

#if HAL_QUADPLANE_ENABLED
    if (plane.quadplane.show_vtol_view()) {
        // VTOL view'dan gerçek değerleri al
        true_roll = plane.quadplane.ahrs_view->roll;
        true_pitch = plane.quadplane.ahrs_view->pitch;
        y = plane.quadplane.ahrs_view->yaw;

        // (İstersen burada da trim uygulaması aynı mantıkla true_pitch'e kalır;
        // yukarıda zaten uygulandı, ama VTOL view için ayrıca istiyorsan burada da ekleyebilirsin.)
    }
#endif

    // SWAP: MAVLink roll alanına gerçek pitch, MAVLink pitch alanına gerçek roll gönder
    const float out_roll = true_pitch;
    const float out_pitch = true_roll;

    const Vector3f& omega = ahrs.get_gyro();
    // SWAP: rollspeed <-> pitchspeed
    mavlink_msg_attitude_send(
        ...
        out_pitch,
        y,
        omega.y,   // rollspeed  = gerçek pitch rate
        omega.x,   // pitchspeed = gerçek roll rate
        omega.z);
}
